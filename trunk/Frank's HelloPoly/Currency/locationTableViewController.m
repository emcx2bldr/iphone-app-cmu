...
-(BOOL) pairIsComplete{
	return (selectedPair.fromCurrency != nil && selectedPair.toCurrency != nil);
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	NSLog(@"%@",self.selectedPair.toCurrency.name);
	
	if (self.selectedPair.toCurrency == nil) {
		...
		NSLog(@"%@", newPair.fromCurrency.name);
		locationTableViewController *from = [[locationTableViewController alloc] initWithPair: newPair];
		[newPair release];
		NSLog(@"%@",from.selectedPair.toCurrency.name);
	...
	else
	{
		...
		result.selectedPair.fromCurrency = (currency *)[self.currencyArray objectAtIndex: indexPath.row];
